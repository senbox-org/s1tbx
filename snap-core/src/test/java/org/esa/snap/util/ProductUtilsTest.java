...
public class ProductUtilsTest {
    ...
    public void testTransformToProductGrid_PixelPos() throws TransformException, SceneRasterTransformException {
        ...
        TiePointGrid lat2 = new TiePointGrid("lat2", 2, 2, 0, 0, 2, 4, new float[]{1f, 5f, 1f, 5f});
        TiePointGrid lon2 = new TiePointGrid("lon2", 2, 2, 0, 0, 2, 4, new float[]{1f, 1f, 9f, 9f});
        ...
        for (double y = 0.5; y < 8; y++) {
            for (double x = 0.5; x < 4; x++) {
                ...
                final PixelPos productPos = ProductUtils.transformToProductGrid(band1, pixelPos);
                assertEquals(pixelPos.getX(), productPos.getX(), 1e-8);
                assertEquals(pixelPos.getY(), productPos.getY(), 1e-8);
            }
        }
        for (double y = 0; y < 4; y += 0.5) {
            for (double x = 0; x < 2; x += 0.5) {
                final PixelPos pixelPos = new PixelPos(x, y);
                final PixelPos productPos = ProductUtils.transformToProductGrid(band2, pixelPos);
                assertEquals(pixelPos.getX() * 2, productPos.getX(), 1e-8);
                assertEquals(pixelPos.getY() * 2, productPos.getY(), 1e-8);
            }
        }
    }
    ...
    public void testTransformToRasterGrid_PixelPos() throws TransformException, SceneRasterTransformException {
        ...
        for (double y = 0; y < 8; y++) {
            for (double x = 0; x < 4; x++) {
                final PixelPos pixelPos = new PixelPos(x, y);
                final PixelPos productPos = ProductUtils.transformToRasterGrid(band2, pixelPos);
                assertEquals(pixelPos.getX() / 2, productPos.getX(), 1e-8);
                assertEquals(pixelPos.getY() / 2, productPos.getY(), 1e-8);
            }
        }
    ...
    public void testTransformFromToRasterGrid_PixelPos() throws TransformException, SceneRasterTransformException {
        ...
        product.addTiePointGrid(lat2);
        product.addTiePointGrid(lon2);
        TiePointGrid lat3 = new TiePointGrid("lat3", 2, 2, 0, 0, 3, 10, new float[]{1f, 5f, 1f, 5f});
        TiePointGrid lon3 = new TiePointGrid("lon3", 2, 2, 0, 0, 3, 10, new float[]{1f, 1f, 9f, 9f});
        product.addTiePointGrid(lat3);
        product.addTiePointGrid(lon3);
        Band band1 = new VirtualBand("B1", ProductData.TYPE_INT16, 3, 10, "X");
        Band band2 = new VirtualBand("B2", ProductData.TYPE_INT16, 2, 4, "Y");
        product.addBand(band1);
        product.addBand(band2);
        ...
        for (double y = 0; y < 9; y++) {
            for (double x = 0; x < 3; x++) {
                ...
                final PixelPos transformedBand1Pos = ProductUtils.transformFromToRasterGrid(band1, band2, band1Pos);
                assertEquals(band1Pos.getX() * oneThird, transformedBand1Pos.getX(), 1e-8);
                assertEquals(band1Pos.getY() * 0.4, transformedBand1Pos.getY(), 1e-8);
            }
        }
        for (double y = 0; y < 4; y++) {
            for (double x = 0; x < 2; x++) {
                final PixelPos band2Pos = new PixelPos(x, y);
                final PixelPos transformedBand2Pos = ProductUtils.transformFromToRasterGrid(band2, band1, band2Pos);
                assertEquals(band2Pos.getX() * 1.5, transformedBand2Pos.getX(), 1e-8);
                assertEquals(band2Pos.getY() * 2.5, transformedBand2Pos.getY(), 1e-8);
            }
        }
    }
    ...
    @Test
        ...
        Shape transformedShape = ProductUtils.transformToProductGrid(band2, polygon);
        assertEquals(true, transformedShape instanceof GeneralPath);
        ...
        assertEquals(1.0, currentSegment[0], 1e-8);
        assertEquals(1.0, currentSegment[1], 1e-8);
        transformedShapePathIterator.next();
        transformedShapePathIterator.currentSegment(currentSegment);
        assertEquals(3.0, currentSegment[0], 1e-8);
        assertEquals(2.0, currentSegment[1], 1e-8);
        transformedShapePathIterator.next();
        transformedShapePathIterator.currentSegment(currentSegment);
        assertEquals(3.0, currentSegment[0], 1e-8);
        assertEquals(6.0, currentSegment[1], 1e-8);
        transformedShapePathIterator.next();
        transformedShapePathIterator.currentSegment(currentSegment);
        assertEquals(2.0, currentSegment[0], 1e-8);
        assertEquals(7.0, currentSegment[1], 1e-8);
    }
    ...
    public void testTransformToRasterGrid_Shape() throws TransformException, SceneRasterTransformException {
        ...
        int[] ypoints = {1, 2, 6, 7};
        Polygon polygon = new Polygon(xpoints, ypoints, 4);
        Shape transformedShape = ProductUtils.transformToRasterGrid(band2, polygon);
        assertEquals(true, transformedShape instanceof GeneralPath);
        PathIterator transformedShapePathIterator = transformedShape.getPathIterator(null);
        double[] currentSegment = new double[2];
        transformedShapePathIterator.currentSegment(currentSegment);
        ...
        transformedShapePathIterator.next();
        transformedShapePathIterator.currentSegment(currentSegment);
        assertEquals(6.0, currentSegment[0], 1e-8);
        assertEquals(4.0, currentSegment[1], 1e-8);
        transformedShapePathIterator.next();
        transformedShapePathIterator.currentSegment(currentSegment);
        assertEquals(6.0, currentSegment[0], 1e-8);
        assertEquals(12.0, currentSegment[1], 1e-8);
        transformedShapePathIterator.next();
        transformedShapePathIterator.currentSegment(currentSegment);
        assertEquals(4.0, currentSegment[0], 1e-8);
        assertEquals(14.0, currentSegment[1], 1e-8);
    }
    ...
    public void testTransformFromToRasterGrid_Shape() throws TransformException, SceneRasterTransformException {
        ...
        Polygon polygon = new Polygon(xpoints, ypoints, 4);
        Shape transformedShape = ProductUtils.transformFromToRasterGrid(band1, band2, polygon);
        assertEquals(true, transformedShape instanceof GeneralPath);
        PathIterator transformedShapePathIterator = transformedShape.getPathIterator(null);
        ...
        transformedShapePathIterator.currentSegment(currentSegment);
        assertEquals(2.0, currentSegment[0], 1e-8);
        assertEquals(2 * oneThird, currentSegment[1], 1e-7);
        transformedShapePathIterator.next();
        transformedShapePathIterator.currentSegment(currentSegment);
        assertEquals(3.0 + oneThird, currentSegment[0], 1e-6);
        assertEquals(6.0, currentSegment[1], 1e-8);
        transformedShapePathIterator.next();
        transformedShapePathIterator.currentSegment(currentSegment);
        assertEquals(2.0 + (2 * oneThird), currentSegment[0], 1e-7);
        assertEquals(7.0 + oneThird, currentSegment[1], 1e-6);
        transformedShapePathIterator.next();
        transformedShapePathIterator.currentSegment(currentSegment);
        assertEquals(1.0 + oneThird, currentSegment[0], 1e-7);
        assertEquals(2.0, currentSegment[1], 1e-8);
        ...
        transformedShape = ProductUtils.transformFromToRasterGrid(band2, band1, polygon);
        assertEquals(true, transformedShape instanceof GeneralPath);
        transformedShapePathIterator = transformedShape.getPathIterator(null);
        transformedShapePathIterator.currentSegment(currentSegment);
        assertEquals(1.5, currentSegment[0], 1e-8);
        assertEquals(3.0, currentSegment[1], 1e-8);
        transformedShapePathIterator.next();
        transformedShapePathIterator.currentSegment(currentSegment);
        assertEquals(4.5, currentSegment[0], 1e-8);
        assertEquals(10.5, currentSegment[1], 1e-8);
        transformedShapePathIterator.next();
        transformedShapePathIterator.currentSegment(currentSegment);
        assertEquals(4.5, currentSegment[0], 1e-8);
        assertEquals(9.0, currentSegment[1], 1e-8);
        transformedShapePathIterator.next();
        transformedShapePathIterator.currentSegment(currentSegment);
        assertEquals(3.0, currentSegment[0], 1e-8);
        assertEquals(1.5, currentSegment[1], 1e-8);
    }
