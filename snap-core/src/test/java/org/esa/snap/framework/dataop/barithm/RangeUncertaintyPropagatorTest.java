...
public class RangeUncertaintyPropagatorTest  extends UncertaintyPropagatorTest {
    ...
    public void testRangeUncertaintyPropagator() throws Exception {
        ...
        //assertEquals("Add(Div(B1_unc,abs(B1)),Div(B2_unc,abs(B2)))", uncert(product, "B1 * B2 * B3"));
        //assertEquals("Add(Div(B1_unc,abs(B1)),Div(B2_unc,abs(B2)))", uncert(product, "B2 * B3 * B1"));
        ...
        assertEquals("abs(0.0)", uncert("sin(B3)"));
        assertEquals("abs(Sub(sin(Sub(B1,B1_unc)),sin(Add(B1,B1_unc))))", uncert("sin(B1)"));
        assertEquals("abs(Sub(sqrt(Sub(B1,B1_unc)),sqrt(Add(B1,B1_unc))))", uncert("sqrt(B1)"));
    }
