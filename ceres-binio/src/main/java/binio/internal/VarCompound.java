...
final class VarCompound extends AbstractCompound {
    ...
    public VarCompound(IOContext context, CollectionData parent, CompoundType compoundType, long position) {
        ...
        // todo - OPT: idea: register a hint with a type in a format whcih tells the binio API to use a fixed size segment

        // Determine first members up to maxMemberIndex which have known size
        int maxMemberIndex = FixCompound.getMemberIndexWithinSizeLimit(compoundType, binio.internal.Segment.SEGMENT_SIZE_LIMIT);
        if (maxMemberIndex >= 0) {
            int segmentSize = 0;
