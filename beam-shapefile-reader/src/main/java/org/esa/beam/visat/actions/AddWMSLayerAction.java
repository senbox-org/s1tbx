...
public class AddWMSLayerAction extends ExecCommand {
    ...
    private void importWMS(final VisatApp visatApp) {
    ...
    public static void localWMS(final VisatApp visatApp) throws Exception {
        final ProductSceneView view = visatApp.getSelectedProductSceneView();
        if (view == null) {
            return;
        }

        URL url = new URL("http://www.geovista.psu.edu/geoserver/wms");
//        URL url = new URL("http://localhost:8080/geoserver/wms?REQUEST=GetCapabilities");
		...
		org.geotools.data.ows.Layer layer = null;
        for (org.geotools.data.ows.Layer test : caps.getLayerList()) {
       ...
       //children.add(children.size() - 1, wmslayer);
