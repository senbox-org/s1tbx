...
//        // _____ ovsf=16-->apriorisigma=0.03
        ...
        while (DONE != 1) {
            ...
            for (i = 0; i < Nobs; i++) {
                ...
                logger.info("coregpm: (" + posL + ", " + posP + "): yL=" + yL.get(i, 0) + " yP=" + yP.get(i, 0));
...
//            maxdev = max(abs(N * Qx_hat - eye(real8(Qx_hat.lines()))));
            ...
            if (maxdev > .01) {
                logger.severe("coregpm: maximum deviation N*inv(N) from unity = {}. This is larger than 0.01"+ maxdev);
                throw new IllegalStateException("coregpm: maximum deviation N*inv(N) from unity)");
            } else if (maxdev > .001) {
                logger.warning("coregpm: maximum deviation N*inv(N) from unity = {}. This is between 0.01 and 0.001"+ maxdev);
            }
