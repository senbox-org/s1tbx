...
public class NioFile extends File {
     ...
     * @see java.io.File#isAbsolute()
     */
    private Path getAbsolutePath_p() {
    ...
    public File getAbsoluteFile() {
        Path absPath = getAbsolutePath_p();
        try {
     ...
     * pathname after the file or directory is deleted.
     *
     * @return The canonical path denoting the same file or
     * directory as this abstract pathname
     * @throws IOException       If an I/O error occurs, which is possible because the
     *                           construction of the canonical pathname may require
     *                           filesystem queries
     ...
     */
    @NotNull
    private Path getCanonicalPath_p() throws IOException {
        if (isInvalid()) {
            throw new IOException("Invalid file path");
        }
    ...
    public File getCanonicalFile() throws IOException {
        Path canonPath = getCanonicalPath_p();
        try {
    ...
    /**
     * Converts this abstract pathname into a <code>scheme:</code> URL.  The
    ...
    public URL toURL() throws MalformedURLException {
        ...
        return new URL(path.toUri().getScheme(), "", slashify(getAbsolutePath_p(), isDirectory()));
    }
     ...
     *
     * <blockquote><tt>
     * new {@link #NioFile(java.net.URI) File}(</tt><i>&nbsp;f</i><tt>.toURI()).equals(</tt><i>&nbsp;f</i><tt>.{@link #getAbsoluteFile() getAbsoluteFile}())
     * </tt></blockquote>
     * <p>
    ...
    @NotNull
    public URI toURI() {
        return fs.getPath(slashify(getAbsolutePath_p(), isDirectory())).toUri();
    }
    ...
    public boolean canRead() {
        return !isInvalid() && Files.isReadable(path);
    }
    ...
    public boolean canWrite() {
        return !isInvalid() && Files.isWritable(path);
    }
    ...
    public boolean exists() {
        return !isInvalid() && canRead() && Files.exists(path);
    }
    ...
    public boolean isDirectory() {
        return !isInvalid() && canRead() && Files.isDirectory(path);
    }
    ...
    public boolean isFile() {
        return !isInvalid() && canRead() && Files.isRegularFile(path);
    }
    ...
    public boolean isHidden() {
        try {
            return !isInvalid() && canRead() && Files.isHidden(path);
        } catch (IOException e) {
            return false;
        }
    }
     ...
     */
    public String[] list() {
        DirectoryStream.Filter<Path> filter_p = entry -> true;
        return list(filter_p);
    }
    ...
    public String[] list(FilenameFilter filter) {
        DirectoryStream.Filter<Path> filter_p = entry -> !(filter.accept(NioFile.this, new File(entry.toString()).getName()));
        return list(filter_p);
    }
     ...
     *
     * @param filter A DirectoryStream.Filter<Path> filter
     * @return An array of abstract DirectoryStream.Filter<Path> denoting the files and
     ...
     * denoting the directory itself and the directory's parent directory are
     * not included in the result. Therefore if this
    ...
    public File[] listFiles(FileFilter filter) {
        DirectoryStream.Filter<Path> filter_p = entry -> !(filter.accept(new File(entry.toString())));
        return listFiles(filter_p);
    }
    ...
    public boolean renameTo(File dest) {
        ...
        try {
            return !this.isInvalid() && !((NioFile) dest).isInvalid() && !Files.move(path, path.resolveSibling(dest.toPath())).toString().isEmpty();
        } catch (IOException e) {
            return false;
        }
    }
    ...
    public boolean setLastModified(long time) {
        ...
        try {
            return !isInvalid() && Files.setLastModifiedTime(path, FileTime.fromMillis(time)) != null;
        } catch (IOException e) {
            return false;
        }
    }
    ...
    public boolean setReadOnly() {
        try {
            ...
            return !isInvalid() && Files.setPosixFilePermissions(path, perms) != null;
        } catch (IOException e) {
            return false;
        }
    }
    ...
    public boolean canExecute() {
        return !isInvalid() && Files.isExecutable(path);
    }
