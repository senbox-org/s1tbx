...
public class ObjectStorageFileChannel extends FileChannel {
    ...
    public long transferTo(long position, long count, WritableByteChannel target) throws IOException {
        ...
        SeekableByteChannel byteChannel = path.getFileSystem().provider().newByteChannel(path, options, attrs);
        if (position > byteChannel.size()) {
            return 0;
        }
    ...
    public MappedByteBuffer map(MapMode mode, long position, long size) throws IOException {
        ...
        int length = (int) size;
        SeekableByteChannel byteChannel = path.getFileSystem().provider().newByteChannel(path, options, attrs);
        byteChannel = byteChannel.position(position);
        ByteBuffer dst = ByteBuffer.allocate(length);
        byteChannel.read(dst);
