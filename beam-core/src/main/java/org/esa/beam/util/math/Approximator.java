/*
 * $Id: Approximator.java,v 1.1.1.1 2006/09/11 08:16:47 norman Exp $
 *
...
public class Approximator {
    ...
    /**
     * Solves a linear equation system with each term having the form p * f(x). The method finds the coefficients
     * <code>p[0]</code> to <code>p[n-1]</code> with <code>n = f.length</code> for an approximation function y'(x) =
     * <code>p[0]*f[0](x) + p[1]*f[1](x) + p[2]*f[2](x) + ... + p[n-1]*f[n-1](x)</code> which approximates the given
     * data vector x<sub>i</sub>=<code>data[i][0]</code>, y<sub>i</sub>=<code>data[i][1]</code> with i = <code>0</code>
     * to <code>data.length-1</code>.
     *
     * @param data    an array of values of the form <code>{{x1,y1}, {x2,y2}, {x3,y3}, ...} </code>
     ...
     *                then <code>indices</code> defaults to <code>{0, 1}</code>.
     * @param f       the function vector, each function has the form y=f(x)
     * @param p       the resulting coefficient vector, must have the same size as the function vector
     * @param eps     the epsilon value, not used anymore
     *
     * @deprecated use {@link #approximateFX(double[][], int[], FX[], double[])} instead
     */
    public static void approximateFX(final double data[][],
                                     int[] indices,
                                     final FX[] f,
                                     final double[] p,
                                     final double eps) {
        approximateFX(data, indices, f, p);
    }

    /**
     * Solves a linear equation system with each term having the form p * f(x,y). The method finds the coefficients
     * <code>p[0]</code> to <code>p[n-1]</code> with <code>n = f.length</code> for an approximation function z'(x,y) =
     * <code>p[0]*f[0](x,y) + p[1]*f[1](x,y) + p[2]*f[2](x,y) + ... + p[n-1]*f[n-1](x,y)</code> which approximates the
     * given data vector x<sub>i</sub>=<code>data[i][0]</code>, y<sub>i</sub>=<code>data[i][1]</code>,
     * z<sub>i</sub>=<code>data[i][2]</code> with i = <code>0</code> to <code>data.length-1</code>.
     *
     * @param data    an array of values of the form <code>{{x1,y1,z1}, {x2,y2,z2}, {x3,y3,z3}, ...} </code>
     ...
     * @param eps     the epsilon value, not used anymore
     *
     * @deprecated use {@link #approximateFXY(double[][], int[], FXY[], double[])} instead
     */
    public static void approximateFXY(final double data[][],
                                      int[] indices,
                                      final FXY[] f,
                                      final double[] p,
                                      final double eps) {
        approximateFXY(data, indices, f, p);
    }
