    ...
    static class NearestNeighbour extends LongDataInterpolator {
        ...
        public void interpolate(Rectangle destRect, Rectangle srcRect, double scaleX, double scaleY,
            ...
            for (int dstY = 0; dstY < destRect.getHeight(); dstY++) {
                ...
                final boolean yValid = srcYF >= 0 && srcYF < srcRect.getHeight();
                int srcY = getSrcOffset() + (int) srcYF * getSrcScalineStride();
                for (int dstX = 0; dstX < destRect.getWidth(); dstX++) {
                    double srcXF = offsetX + (scaleX * dstX);
                    int srcX = (int) srcXF;
                    if (yValid && srcX >= 0 && srcX < srcRect.getWidth()) {
                        setDstData(dstIndexY + dstX, getSrcData(srcY + srcX));
                    } else {
                        setDstData(dstIndexY + dstX, 2000);
                    }
                }
