...
public class AESBEOS2010CropModelDescriptor extends AbstractLandCoverModelDescriptor {
    ...
    private static final File INSTALL_DIR = new File(Settings.instance().getAuxDataFolder().getAbsolutePath(),
            "LandCover" + File.separator + "AAFC" + File.separator + "AESB_EOS_Crop_2010");
    private static final File[] fileList = new File[]{
            new File(INSTALL_DIR, "AAFC_Prairie_Crop_Mapping_2010_v3.zip")
    };

    public AESBEOS2010CropModelDescriptor() {
        remotePath = "http://nest.s3.amazonaws.com/data/LandCover/AAFC/AESB_EOS_Crop_2010/";
        name = NAME;
        installDir = INSTALL_DIR;
        NO_DATA_VALUE = -9999d;
        metadataFileName = "AAFC_Prairie_Crop_Mapping_2010_v3_Metadata_EN.xml";
        final Path moduleBasePath = ResourceInstaller.findModuleCodeBasePath(this.getClass());
        metadataSrcPath = moduleBasePath.resolve("org/esa/snap/landcover/auxdata/aafc/AAFC_Prairie_Crop_Mapping_2010/");
