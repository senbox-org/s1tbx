 ...
 * data. The first command line argument gives the path of the directory holding
 * the training data. The optional second argument, leakType, defines which
 * classes of features to use. Importantly, leakType controls whether a
 * synthetic date is injected into the data as a target leak and if so, how.
 * <p/>
 ...
 * This will be a single token and is a perfect target leak since each newsgroup
 * is given a different month</td></tr>
 * <tr><td valign='top'>2</td><td>Synthetic date injected in dd-MMM-yyyy
 * HH:mm:ss format. The day varies and thus there are more leak symbols that
 * need to be learned. Ultimately this is just as big a leak as case
 * 1.</td></tr>
 * </table>
 * <p/>
 * Leaktype also determines what other text will be indexed. If leakType is
 * greater than or equal to 6, then neither headers nor text body will be used
 * for features and the leak is the only source of data. If leakType is greater
 * than or equal to 3, then subject words will be used as features. If leakType
 * is less than 3, then both subject and body text will be used as features.
 * <p/>
