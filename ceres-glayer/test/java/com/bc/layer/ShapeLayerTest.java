...
public class ShapeLayerTest extends ImagingTestCase {
    ...
    public void testBoundingBox() {
        ...
        assertEquals(new Rectangle2D.Double(0.0, 0.0, 320.0, 200.0), layer.getBoundingBox());

        final AffineTransform s2u = new AffineTransform(0.5, 0, 0, 0.5, -25.5, 50.3);
        layer = new ShapeLayer(new Shape[]{shape}, s2u);
        assertNotNull(layer.getBoundingBox());
        assertEquals(new Rectangle2D.Double(-25.5, 50.3, 160.0, 100.0), layer.getBoundingBox());
    }
}