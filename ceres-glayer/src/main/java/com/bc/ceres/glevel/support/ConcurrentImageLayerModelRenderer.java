...
public class ConcurrentImageLayerModelRenderer implements ImageLayerModelRenderer {
    ...
    public ConcurrentImageLayerModelRenderer() {
        lastLevel = -1;
    ...
    @Override
    public void renderImage(Rendering rendering, ImageLayerModel imageLayerModel, int currentLevel) {
        final long t0 = System.nanoTime();
        renderImpl((InteractiveRendering) rendering, imageLayerModel, currentLevel);
        if (debug) {
            final long t1 = System.nanoTime();
            double time = (t1 - t0) / (1000.0 * 1000.0);
            System.out.printf("ConcurrentImageLayerModelRenderer: render: time=%f ms, clip=%s\n", time, rendering.getGraphics().getClip());
        }
    }

    private void renderImpl(InteractiveRendering rendering, ImageLayerModel imageLayerModel, int currentLevel) {
        ...
        final PlanarImage planarImage = (PlanarImage) imageLayerModel.getLevelImageSource().getLevelImage(currentLevel);
        ...
        final Rectangle clippedImageRegion = getImageRegion(viewport, imageLayerModel, currentLevel, clipBounds);
        ...
        drawTentativeTileImages(graphics, viewport,
                                imageLayerModel, currentLevel, planarImage, missingTileIndexList);
        ...
        // Draw tile frames
        final AffineTransform i2m = imageLayerModel.getImageToModelTransform(currentLevel);
        ...
        final Rectangle visibleImageRegion = getImageRegion(viewport, imageLayerModel, currentLevel, rendering.getBounds());
    ...
    private static Rectangle getImageRegion(Viewport vp, ImageLayerModel imageLayerModel, int level, Rectangle2D viewRegion) {
        return getViewToImageTransform(vp, imageLayerModel, level).createTransformedShape(viewRegion).getBounds();
    }

    private static Rectangle getViewRegion(Viewport vp, ImageLayerModel imageLayerModel, int level, Rectangle2D imageRegion) {
        return getImageToViewTransform(vp, imageLayerModel, level).createTransformedShape(imageRegion).getBounds();
    }

    private static AffineTransform getViewToImageTransform(Viewport vp, ImageLayerModel imageLayerModel, int level) {
        final AffineTransform t = vp.getViewToModelTransform();
        t.preConcatenate(imageLayerModel.getModelToImageTransform(level));
        return t;
    }

    private static AffineTransform getImageToViewTransform(Viewport vp, ImageLayerModel imageLayerModel, int level) {
        final AffineTransform t = new AffineTransform(imageLayerModel.getImageToModelTransform(level));
        t.preConcatenate(vp.getModelToViewTransform());
        return t;
    }
    ...
    private class TileComputationHandler implements TileComputationListener {
        ...
        private final ImageLayerModel imageLayerModel;
        private final int level;

        private TileComputationHandler(InteractiveRendering rendering, ImageLayerModel imageLayerModel, int level) {
        ...
        public void tileComputed(Object object,
            ...
            final TileImage tileImage = createTileImage(deviceConfiguration,
                                                        ...
                                                        tileIndex,
                                                        tile,
                                                        imageLayerModel.getImageToModelTransform(level));
            synchronized (ConcurrentImageLayerModelRenderer.this) {
                scheduledTileRequests.remove(tileIndex);
            ...
            rendering.invokeLater(new Runnable() {
                ...
                @Override
                public void run() {
                    final Rectangle viewRegion = getViewRegion(rendering.getViewport(), imageLayerModel, level, tileBounds);
        ...
        public void tileCancelled(Object object,
            ...
            dropTile(tileIndex);
            if (debug) {
                System.out.printf("ConcurrentImageLayerModelRenderer: tileCancelled: %s\n", tileIndex);
            }
        }
        ...
        public void tileComputationFailure(Object object,
            ...
            dropTile(tileIndex);
            if (debug) {
                System.out.printf("ConcurrentImageLayerModelRenderer: tileComputationFailure: %s\n", tileIndex);
    ...
    private final class TileImageCache {
        ...
        public synchronized void add(TileImage tileImage) {
            ...
            size += tileImage.size;
            if (debug) {
                System.out.printf("ConcurrentImageLayerModelRenderer$TileImageCache: add: tileIndex=%s, size=%d\n", tileImage.tileIndex, size);
            }
        }
        ...
        public synchronized void remove(TileIndex tileIndex) {
            ...
            if (oldTileImage != null) {
                size -= oldTileImage.size;
                if (debug) {
                    System.out.printf("ConcurrentImageLayerModelRenderer$TileImageCache: remove: tileIndex=%s, size=%d\n", tileIndex, size);
                }
            }
        }
