...
public class FileImageLayerModelFactory {

    public static ImageLayerModel create(File location, String extension, AffineTransform imageToModelTransform, int levelCount) {
        final LIS levelImageSource = new LIS(location, location.getName(), extension, levelCount);
        Rectangle2D modelBounds = DefaultImageLayerModel.getModelBounds(imageToModelTransform, levelImageSource.getLevelImage(0));
