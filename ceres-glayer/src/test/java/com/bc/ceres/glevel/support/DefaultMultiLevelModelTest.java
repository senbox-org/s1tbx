...
public class DefaultMultiLevelModelTest extends TestCase {
    ...
    public void testAllProperties() {
        ...
        assertEquals(new Rectangle2D.Double(TX0, TY0, S0 * W + TX0, S0 * H + TY0), model.getModelBounds());
    ...
    private void testLevel(DefaultMultiLevelModel model, int level) {
        ...
        assertEquals(ES[level], scale, 1e-10);
        assertEquals(level, model.getLevel(scale));
        assertEquals(new Point2D.Double(TX0, TY0),
                     i2m.transform(new Point2D.Double(0, 0), null));
        assertEquals(new Point2D.Double(S0 * scale + TX0, S0 * scale + TY0),
