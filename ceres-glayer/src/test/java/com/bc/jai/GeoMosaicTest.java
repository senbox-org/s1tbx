...
//                dir + "l5_20060718_georef_rgb432",
        ...
        for (int i = 0; i < imageFeatures.length; i++) {
            ...
            imageFeatures[i] = new ImageFeature(Utils.loadImage(filePath + ".png"),
                                                Utils.loadWorldFile(filePath + ".pgw"));
        }
        ...
        for (int i = 0; i < sourceImages.length; i++) {
            ...
            sourceImages[i] = Utils.transformImage(imageFeatures[i].image, transform);
        }
        RenderedOp image = Utils.createMosaic(sourceImages);
        Utils.displayImage("GeoMosaicTest", image, new AffineTransform(), 8, true);

        //Utils.storeTiffPyramid(image, "geomosaic", 8);
    }
}