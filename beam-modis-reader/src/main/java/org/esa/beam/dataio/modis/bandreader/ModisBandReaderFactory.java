...
public class ModisBandReaderFactory {
     ...
     * @throws ncsa.hdf.hdflib.HDFException
     */
    public static ModisBandReader[] getReaders(Variable variable, final ModisBandDescription desc) throws HDFException {
        ModisBandReader[] readers = null;
        ...
        final int prodIODataType = NetCdfUtils.decodeDataType(dataType);
        final List<Dimension> dimensions = variable.getDimensions();
        final boolean is3d;
        if (dimensions.size() > 2) {
            readers = new ModisBandReader[dimensions.size()];
            is3d = true;
        } else {
            readers = new ModisBandReader[1];
            is3d = false;
        ...
        for (int i = 0; i < readers.length; i++) {
            if (prodIODataType == ProductData.TYPE_INT8) {
                readers[i] = new ModisInt8BandReader(variable, i, is3d);
            } else if (prodIODataType == ProductData.TYPE_UINT8) {
                if ((scaleMethod == ModisBandReader.SCALE_LINEAR)
                        // @todo IMAPP
                        || (scaleMethod == ModisBandReader.SCALE_UNKNOWN)) {
                    // @todo IMAPP
                    readers[i] = new ModisUint8BandReader(variable, i, is3d);
                } else if (scaleMethod == ModisBandReader.SCALE_EXPONENTIAL) {
                    readers[i] = new ModisUint8ExpBandReader(variable, i, is3d);
                }
            } else if (prodIODataType == ProductData.TYPE_UINT16) {
                if ((scaleMethod == ModisBandReader.SCALE_UNKNOWN) ||
                        (scaleMethod == ModisBandReader.SCALE_LINEAR) ||
                        (scaleMethod == ModisBandReader.SCALE_SLOPE_INTERCEPT)) {
                    readers[i] = new ModisUint16BandReader(variable, i, is3d);
                } else if (scaleMethod == ModisBandReader.SCALE_POW_10) {
                    readers[i] = new ModisUint16PowBandReader(variable, i, is3d);
                }
            } else if (prodIODataType == ProductData.TYPE_INT16) {
                if ((scaleMethod == ModisBandReader.SCALE_UNKNOWN) ||
                        (scaleMethod == ModisBandReader.SCALE_LINEAR)) {
                    readers[i] = new ModisInt16BandReader(variable, i, is3d);
                }
            } else if (prodIODataType == ProductData.TYPE_UINT32) {
                if ((scaleMethod == ModisBandReader.SCALE_UNKNOWN) ||
                        (scaleMethod == ModisBandReader.SCALE_LINEAR) ||
                        (scaleMethod == ModisBandReader.SCALE_SLOPE_INTERCEPT)) {
                    readers[i] = new ModisUint32BandReader(variable, i, is3d);
                }
            }
            readers[i].setName(bandName);
        }
